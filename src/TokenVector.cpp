...
TokenVector::TokenVector(std::string &source) : source(source) {}
...
void TokenVector::addToken(TokenType type, int start, int line)
{
    std::string text = source.substr(start, current - start);
    tokens.emplace_back(type, text, line);
}
void TokenVector::addToken(TokenType type, double literal, int start, int line)
{
    std::string text = source.substr(start, current - start);
    tokens.emplace_back(type, text, literal, line);
}
void TokenVector::addToken(TokenType type, std::string literal, int start, int line)
{
    std::string text = source.substr(start, current - start);
    tokens.emplace_back(type, text, literal, line);
}
void TokenVector::scanToken();
